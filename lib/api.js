...
var Mnemonic = require('proccore-mnemonic');
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.coin || 'proc', opts.network || 'livenet');
};
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.coin || 'proc', opts.network || 'livenet', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromExtendedPrivateKey = function(xPrivKey, opts) {
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPrivateKey(opts.coin || 'proc', xPrivKey, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.coin || 'proc', opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPublicKey(opts.coin || 'proc', xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.coin || 'proc', opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.importFromExtendedPrivateKey = function(xPrivKey, opts, cb) {
  ...
  try {
    this.credentials = Credentials.fromExtendedPrivateKey(opts.coin || 'proc', xPrivKey, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
  } catch (e) {
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['proc', 'bch'], coin));
  $.checkArgument(network || _.includes(['livenet', 'testnet'], network));

  self._doGetRequest('/v2/feelevels/?coin=' + (coin || 'proc') + '&network=' + (network || 'livenet'), function(err, result) {
    if (err) return cb(err);
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'proc';
  if (!_.includes(['proc', 'bch'], coin)) return cb(new Error('Invalid coin'));
